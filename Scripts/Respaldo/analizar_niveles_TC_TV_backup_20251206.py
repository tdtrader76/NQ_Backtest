# Versión 1.1 - 2025-12-06 15:22 - Línea modificada: 72 (sheet_name='Sheet1')
# Corregido nombre de hoja de Excel de 'Datos_Completos' a 'Sheet1'
...
Entrada:
    - Datos_2025_EM21_Niveles.xlsx (hoja 'Datos_Completos')
...
def analizar_TCH_TCL(df):
    ...
    # 1. Toques de TCH y TCL
    toco_TCH = (df['High'] >= df['TCH']).sum()
    toco_TCL = (df['Low'] <= df['TCL']).sum()
    ...
    toco_solo_TCH = ((df['High'] >= df['TCH']) & (df['Low'] > df['TCL'])).sum()
    toco_solo_TCL = ((df['Low'] <= df['TCL']) & (df['High'] < df['TCH'])).sum()
    toco_ambos = ((df['High'] >= df['TCH']) & (df['Low'] <= df['TCL'])).sum()
    ...
    logger.info(f"  Tocó TCH: {stats['toco_TCH']} ({stats['toco_TCH_pct']:.1f}%)")
    logger.info(f"  Tocó TCL: {stats['toco_TCL']} ({stats['toco_TCL_pct']:.1f}%)")
...
def analizar_TVH_TVL(df):
    """
    Analiza niveles TVH/TVL con estadísticas detalladas (análisis simétrico invertido)
    ...
    # 1. Toques de TVH y TVL
    toco_TVH = (df['High'] >= df['TVH']).sum()
    ...
    # 2. Toques exclusivos y ambos
    toco_solo_TVH = ((df['High'] >= df['TVH']) & (df['Low'] > df['TVL'])).sum()
    toco_solo_TVL = ((df['Low'] <= df['TVL']) & (df['High'] < df['TVH'])).sum()
    toco_ambos = ((df['High'] >= df['TVH']) & (df['Low'] <= df['TVL'])).sum()
    ...
    logger.info(f"  Total días analizados: {stats['total_dias']}")
    logger.info(f"  Tocó TVH: {stats['toco_TVH']} ({stats['toco_TVH_pct']:.1f}%)")
    logger.info(f"  Tocó TVL: {stats['toco_TVL']} ({stats['toco_TVL_pct']:.1f}%)")
